...
import 'package:ratings_client/src/generated/ratings_features_user.pb.dart';
...
void main() {
  ...
  final ratingsClient =
      RatingsClient.withClients(mockAppClient, mockUserClient);
  ...
  test('get rating', () async {
    ...
    when(mockAppClient.getRating(request, options: anyNamed('options')))
        .thenAnswer((_) => MockResponseFuture<GetRatingResponse>(mockResponse));
    final response = await ratingsClient.getRating(snapId, token);
    expect(response, equals(mockResponse));
    final capturedArgs = verify(mockAppClient.getRating(request,
            options: captureAnyNamed('options')))
        .captured;
    final capturedOptions = capturedArgs.single as CallOptions;
    expect(capturedOptions.metadata,
  ...
  test('list user votes', () async {
    ...
    when(mockUserClient.listMyVotes(request, options: anyNamed('options')))
        .thenAnswer(
            (_) => MockResponseFuture<ListMyVotesResponse>(mockResponse));
    final response = await ratingsClient.listMyVotes(snapIdFilter, token);
    expect(response, equals(mockResponse));
  ...
  test('user votes', () async {
    ...
    final request =
        VoteRequest(snapId: snapId, snapRevision: snapRevision, voteUp: voteUp);

    when(mockUserClient.vote(request, options: anyNamed('options')))
        .thenAnswer((_) => MockResponseFuture<Empty>(Empty()));
    final response =
        await ratingsClient.vote(snapId, snapRevision, voteUp, token);
    expect(response, equals(mockResponse));
    final capturedArgs = verify(
            mockUserClient.vote(request, options: captureAnyNamed('options')))
        .captured;
    final capturedOptions = capturedArgs.single as CallOptions;
    expect(capturedOptions.metadata,
        containsPair('authorization', 'Bearer $token'));
  ...
  test('delete user', () async {
    ...
    when(mockUserClient.delete(request, options: anyNamed('options')))
        .thenAnswer((_) => MockResponseFuture<Empty>(Empty()));
